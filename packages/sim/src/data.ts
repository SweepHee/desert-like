...
export const MAPS: Record<string, MapDef> = {
   ...
   *
   * 작가가 그린 지형을 통째로 배경으로 깐다(bgImage). 같이 받은 Wang 시트는
   * 길이 타일 「변」으로 이어지는 엣지형이라 엔진의 코너형(WANG_16)과 규약이
   * 달라서 쓰지 않았다 — 억지로 맞추면 그려 준 길 모양이 뭉개진다.
   * 나무·바위는 배경에 미리 합성해 두었고, 집 4채만 부술 수 있는 실물로 남긴다.
   *
   * 적은 북쪽 두 모서리 숲길로 밀려들고, 주민은 서쪽 길로 빠져나간다.
   */
  village: {
    id: 'village',
    name: '자정의 마을',
    length: tiles(56),
    /*
     * 반폭 30 — 원래 21 이었는데 적이 11시·1시 숲길에 들어서자마자 마을에 닿았다.
     * 북쪽에 18타일을 덧대 진입로를 길게 뽑았다 (gen_village3.py).
     * 그래서 마을 자체는 화면 아래쪽에 앉고, 위쪽 절반이 적이 걸어 내려오는 숲이다.
     */
    halfW: tiles(30),
    center: [[0, 0], [tiles(56), 0]],
    bgImage: '/assets/maps/village.png',
    mask: { cols: 120, rows: 112, data: MASK_VILLAGE },
  ...
  greatroot: {
    ...
    mask: { cols: 152, rows: 240, data: MASK_GREATROOT }, // 타일당 4칸 — 손그림의 좁은 길을 살리려면 이 해상도가 필요하다
