 ...
 */
/**
 * 「에메랄드 숲의 값」(15) 통행 마스크 — 유기적 금광 고원 (240x108, 타일당 2칸).
 *
 * 13라운드의 굽은 길 구성과 15라운드 금광 지형을 참조한 원화에 양쪽 오브젝트
 * 시트를 gen_goldmine.py가 합성한다. 길은 위아래로 굽고, 다리와 폐광 우회로에서
...
export const MAPS: Record<string, MapDef> = {
   ...
   *
   * 120×54 대형 고원에 굽은 길·물길·다리·폐광 우회로가 이어진다.
  ...
  goldmine: {
    id: 'goldmine',
    name: '금광 고원',
    // 2880×1296 배경 = 120×54타일. 장거리 이동을 강제하는 전용 전장이다.
    length: tiles(120),
    halfW: tiles(27),
    // 중앙선은 평평하게 둔다 — 갱·집합지 좌표를 그림에서 잰 값 그대로 쓰기 위해서다
    center: [[0, 0], [tiles(120), 0]],
    bgImage: '/assets/maps/goldmine.png',
    mask: { cols: 108, rows: 240, data: MASK_GOLDMINE },
    // 아군 요새는 실제 넥서스, 적 요새는 배경 그림이다(noEnemyNexus).
    nexusX: [tiles(4), tiles(118)],
    nexusPos: [[tiles(4), tiles(1)], [tiles(118), tiles(1)]],
    towerX: [tiles(15), tiles(108)],
    spawnX: [tiles(6), tiles(116)],
    /*
     * 출정 자리를 좌표로 못박는다.
     * 이 판은 집합지를 갱으로 옮겨 다니는데, 그때마다 부대가 갱에서 나오는 것처럼
     * 보이면 안 된다 — 생산은 언제나 요새 앞이고, 거기서 갱까지 걸어가야 한다.
     */
    spawnPos: [[tiles(6), tiles(1)], [tiles(116), tiles(1)]],
  },
  village: {
    id: 'village',
    name: '자정의 마을',
...
reg(D({
  ...
  id: 'k_grandshaman', race: 'karja', name: '천막의 대주술사 하르간', tier: 'final',
