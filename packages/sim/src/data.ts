 ...
 */
/**
 * 「에메랄드 숲의 값」(15) 통행 마스크 — 금광 고원 (112x84, 타일당 2칸).
 *
 * 작가가 준 15라운드/완성.png 에서 packages/client/tools/gen_goldmine.py 가 뽑았다
 * (따뜻한 황토 길·갱 바닥 = 길, 차가운 바위 능선 = 벽). 갱 여섯과 두 진영은
...
export const MAPS: Record<string, MapDef> = {
   ...
   *
   * 바위 능선 사이로 좁은 길이 굽이돌고, 그 사이에 금광 여섯이 파여 있다.
   * 왼쪽 끝이 우리 야영지, 오른쪽 끝이 카르자 진영이다. 통행 22% 라 길이 좁다 —
   * 비행은 마스크를 무시하므로 능선을 그대로 가로지른다 (하늘이 훨씬 짧다).
   * 배경은 작가 그림 그대로(bgImage), 마스크는 gen_goldmine.py 가 뽑는다.
   */
  goldmine: {
    id: 'goldmine',
    name: '금광 고원',
    ...
    length: tiles(56),
    halfW: tiles(21),
    center: [[0, tiles(-3.4)], [tiles(56), tiles(-3.4)]],
    bgImage: '/assets/maps/goldmine.png',
    mask: { cols: 84, rows: 112, data: MASK_GOLDMINE },
    // 야영지 x5.3 / 카르자 진영 x49.0 (둘 다 중앙선 y -3.4)
    nexusX: [tiles(5.3), tiles(49)],
    towerX: [tiles(16), tiles(40)],
    spawnX: [tiles(9), tiles(45)],
  },
  village: {
    id: 'village',
    name: '자정의 마을',
