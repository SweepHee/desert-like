...
export interface Entity {
  ...
  dotDps: number;
  /**
   * 이 틱까지 「중독」에 다시 걸리지 않는다 — 치유를 받으면 붙는다.
   *
   * 역병 늪(4 독이 스민 숲)은 밟는 동안 매 틱 독을 새로 얹으므로, 치유가
   * 독을 씻어내기만 해서는 다음 틱에 그대로 다시 걸려 힐러가 값을 못 했다.
...
export interface Game {
  ...
  rallyY: number;
  /**
   * 집결 지점의 「머무는 반경」 (FP). 0 이면 기본값(1.2타일).
   *
   * 예전엔 전원이 정확히 한 점을 향해 계속 걸었다. 스무 기가 같은 칸을 밀며
   * 영원히 자리다툼을 해서, 모이려고 애쓰는 것처럼만 보이고 진형이 안 잡혔다.
   * 반경 안에 들어오면 멈춰 세우면 겹침 해소가 알아서 고르게 펴 준다.
   */
  rallyR: number;
  /**
